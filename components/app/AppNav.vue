      ...
      <ul class="p-4 lg:py-8 lg:pl-0 lg:pr-8">
        ...
        <li v-for="(docs, category, index) in categories" :key="category" class="mb-4" :class="{
          ...
          <p v-if="category" class="mb-2 text-gray-500 uppercase tracking-wider font-bold text-sm lg:text-xs">
            {{ category }}</p>
          <ul>
            <li v-for="doc of docs" :key="doc.slug" class="text-gray-700 dark:text-gray-300" v-show="doc.menuTitle">
        ...
        <li class="lg:hidden space-x-2">
          ...
          <div class="flex items-center space-x-4">
            <a v-if="settings.twitter" :href="`https://twitter.com/${settings.twitter}`" target="_blank"
            ...
            <a v-if="settings.github" :href="githubUrls.repo" target="_blank" rel="noopener noreferrer" title="Github"
              name="Github" class="inline-flex text-gray-700 dark:text-gray-300 hover:text-primary-500">
